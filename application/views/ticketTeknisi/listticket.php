				...
				<table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
					...
					<tbody>
						<?php $no = 1; foreach ($ticket as $row){?>
							<tr>
								<td><?= $no ?></td>
								<td><a href="<?= site_url('ticket_teknisi/detail/'.$row->id_ticket)?>" class="font-weight-bold" title="Detail"><?= $row->id_ticket?></a></td>
								<?php if ($row->status == 0){?>
								...
								<?php } ?>
								<td>
									<a href="<?= site_url('ticket_teknisi/detail/'.$row->id_ticket)?>" class="btn btn-primary btn-circle btn-sm" title="Detail">
